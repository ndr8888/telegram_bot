...
def run_game():
    def run_life():
        ...
        player_image = pygame.transform.scale(load_image('ufo.png'), (PLAYER_WIDTH, PLAYER_HEIGHT))
    ...
    def end_screen(score):
        try:
            with open('game/high_score_encoded.txt', mode='rb') as f:
                high_score = int(
                    encrypt1(f.read(), b'iSHdbvjhgiucascoafukywedugbqx').decode('utf-8'))
            if score > high_score:
                high_score = score
                with open('game/high_score_encoded.txt', mode='wb') as f:
                    f.write(
                        encrypt1(str(high_score).encode('utf-8'), b'iSHdbvjhgiucascoafukywedugbqx'))
